...
> **v1.0.1 新增（Day 1）**: `Color::fromHex()` 静态方法、`DataPoint` 结构体。均为纯新增，不破坏 v1.0 兼容性。

```cpp
// ==========================================
// 文件: include/xyplot/types.h
// 冻结版本: v1.0.1-frozen (+Color::fromHex, +DataPoint)
// ==========================================
...
| 版本 | 日期 | 变更 | 审批 |
|------|------|------|------|
| v1.0-frozen | 2026-05-31 09:00 | 初始冻结：8 P0 纯虚 + 1 可降级 + types.h 基础类型 | Project Lead |
| **v1.0.1-frozen** | **2026-05-31 17:30** | **+ `Color::fromHex(uint32_t)` 静态方法（内联实现）**  \n**+ `DataPoint` 结构体（数据空间点）**  \n均为纯新增，零破坏性。gate-check ✅ | **Agent A 终审 / Project Lead 批准** |
